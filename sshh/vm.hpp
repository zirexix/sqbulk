    ...
    public:
        ...
        */
        ...
        Object callFunc(const Function& func, const Object& env, Args&&... args) const {
            ...
            auto top = SQ_PTRS->sq_gettop(vm);
            SQ_PTRS->sq_pushobject(vm, func.getRaw());
            SQ_PTRS->sq_pushobject(vm, env.getRaw());
        ...
        */
        Instance newInstanceNoCtor(const Class& cls) const {
            Instance inst(vm);
            SQ_PTRS->sq_pushobject(vm, cls.getRaw());
            SQ_PTRS->sq_createinstance(vm, -1);
            SQ_PTRS->sq_remove(vm, -2);
            SQ_PTRS->sq_getstackobj(vm, -1, &inst.getRaw());
            SQ_PTRS->sq_addref(vm, &inst.getRaw());
            SQ_PTRS->sq_pop(vm, 1);
            return inst;
        }
        /**
