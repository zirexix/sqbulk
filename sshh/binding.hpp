...
namespace ssq {
    ...
    namespace detail {
        ...
        static void bindUserData(HSQUIRRELVM vm, const std::function<Ret(Args...)>& func) {
            auto funcStruct = reinterpret_cast<detail::FuncPtr<Ret(Args...)>*>(SQ_PTRS->sq_newuserdata(vm, sizeof(detail::FuncPtr<Ret(Args...)>)));
            funcStruct->ptr = new std::function<Ret(Args...)>(func);
            SQ_PTRS->sq_setreleasehook(vm, -1, &detail::funcReleaseHook<Ret, Args...>);
        }
        ...
        static Object addClass(HSQUIRRELVM vm, const char* name, const std::function<T*(Args...)>& allocator, bool release = true) {
            ...
            SQ_PTRS->sq_newclass(vm, false);

            HSQOBJECT obj;
            SQ_PTRS->sq_getstackobj(vm, -1, &obj);
            addClassObj(vm, hashCode, obj);

            SQ_PTRS->sq_getstackobj(vm, -1, &clsObj.getRaw());
            SQ_PTRS->sq_addref(vm, &clsObj.getRaw());

            SQ_PTRS->sq_settypetag(vm, -1, reinterpret_cast<SQUserPointer>(hashCode));

            SQ_PTRS->sq_pushstring(vm, "constructor", -1);
            bindUserData<T*>(vm, allocator);
            static char params[33];
            paramPacker<T*, Args...>(params);

            if (release) {
                SQ_PTRS->sq_newclosure(vm, &detail::classAllocator<T, Args...>, 1);
            } else {
                SQ_PTRS->sq_newclosure(vm, &detail::classAllocatorNoRelease<T, Args...>, 1);
            }
        ...
        struct func {
            static SQInteger global(HSQUIRRELVM vm) {
                try {
                    ...
                    return SQ_PTRS->sq_throwerror(vm, e.what());
                }
            }
        };
        ...
        static void addFunc(HSQUIRRELVM vm, const char* name, const std::function<R(Args...)>& func) {
            ...
            SQ_PTRS->sq_newclosure(vm, &detail::func<1, R, Args...>::global, 1);
            SQ_PTRS->sq_setparamscheck(vm, nparams + 1, params);
            if(SQ_FAILED(SQ_PTRS->sq_newslot(vm, -3, SQFalse))) {
        ...
        static void addMemberFunc(HSQUIRRELVM vm, const char* name, const std::function<R(Args...)>& func, bool isStatic) {
            ...
            SQ_PTRS->sq_newclosure(vm, &detail::func<0, R, Args...>::global, 1);
            SQ_PTRS->sq_setparamscheck(vm, nparams, params);
            if(SQ_FAILED(SQ_PTRS->sq_newslot(vm, -3, isStatic))) {
