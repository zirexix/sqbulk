...
namespace ssq {
    ...
    namespace detail {
        ...
        inline void checkType(HSQUIRRELVM vm, SQInteger index, SQObjectType expected){
            auto type = SQ_PTRS->sq_gettype(vm, index);
        ...
        inline T popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            if(type == OT_USERDATA) {
                SQ_PTRS->sq_getuserdata(vm, index, &ptr, &typetag);
                ...
                SQ_PTRS->sq_getinstanceup(vm, index, &ptr, &typetag);
                SQ_PTRS->sq_gettypetag(vm, index, &typetag);
        ...
        inline T popPointer(HSQUIRRELVM vm, SQInteger index) {
            ...
            if(type == OT_USERPOINTER) {
                SQUserPointer val;
                if (SQ_FAILED(SQ_PTRS->sq_getuserpointer(vm, index, &val))) {
                ...
                if (SQ_FAILED(SQ_PTRS->sq_getinstanceup(vm, index, &val, nullptr))) {
                    throw TypeException("Could not get instance from squirrel stack");
                }
                return reinterpret_cast<T>(val);
            }
        ...
        inline Object popValue(HSQUIRRELVM vm, SQInteger index){
            Object val(vm);
            if (SQ_FAILED(SQ_PTRS->sq_getstackobj(vm, index, &val.getRaw()))) throw TypeException("Could not get Object from squirrel stack");
        ...
        inline char popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get char from squirrel stack");
        ...
        inline signed char popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get signed char from squirrel stack");
        ...
        inline short popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get short from squirrel stack");
        ...
        inline int popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get int from squirrel stack");
        ...
        inline long popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get long from squirrel stack");
        ...
        inline unsigned char popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get unsigned char from squirrel stack");
        ...
        inline unsigned short popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get unsigned short from squirrel stack");
        ...
        inline unsigned int popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get unsigned int from squirrel stack");
        ...
        inline unsigned long popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get unsigned long from squirrel stack");
        ...
        inline long long popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get long long from squirrel stack");
        ...
        inline unsigned long long popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQInteger val;
            if (SQ_FAILED(SQ_PTRS->sq_getinteger(vm, index, &val))) throw TypeException("Could not get unsigned long long from squirrel stack");
        ...
        inline double popValue(HSQUIRRELVM vm, SQInteger index){
            checkType(vm, index, OT_INTEGER);
            SQFloat val;
            if (SQ_FAILED(SQ_PTRS->sq_getfloat(vm, index, &val))) throw TypeException("Could not get double from squirrel stack");
        ...
        inline float popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            SQFloat val;
            if (SQ_FAILED(SQ_PTRS->sq_getfloat(vm, index, &val))) throw TypeException("Could not get float from squirrel stack");
        ...
        inline bool popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            SQBool val;
            if (SQ_FAILED(SQ_PTRS->sq_getbool(vm, index, &val))) throw TypeException("Could not get bool from squirrel stack");
            return val == 1;
        }
        ...
        inline std::wstring popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            const SQChar* val;
            if (SQ_FAILED(SQ_PTRS->sq_getstring(vm, index, &val))) throw TypeException("Could not get string from squirrel stack");
            ...
            SQInteger len = SQ_PTRS->sq_getsize(vm,index);
        ...
        inline void pushByCopy(HSQUIRRELVM vm, const T& value) {
            ...
            try {
                SQ_PTRS->sq_pushobject(vm, getClassObj(vm, hashCode));
                SQ_PTRS->sq_createinstance(vm, -1);
                SQ_PTRS->sq_remove(vm, -2);

                SQ_PTRS->sq_setinstanceup(vm, -1, reinterpret_cast<SQUserPointer>(new T(value)));
                SQ_PTRS->sq_settypetag(vm, -1, reinterpret_cast<SQUserPointer>(hashCode));
                SQ_PTRS->sq_setreleasehook(vm, -1, classDestructor<T>);
            } catch (std::out_of_range& e) {
                (void)e;
                T** data = reinterpret_cast<T**>(SQ_PTRS->sq_newuserdata(vm, sizeof(T*)));
                *data = new T(value);
                SQ_PTRS->sq_setreleasehook(vm, -1, classPtrDestructor<T>);
                SQ_PTRS->sq_settypetag(vm, -1, reinterpret_cast<SQUserPointer>(typeid(T).hash_code()));
            }
        }
        ...
        inline void pushValue(HSQUIRRELVM vm, const bool& value) {
            SQ_PTRS->sq_pushbool(vm, value);
        }

        template<>
        inline void pushValue(HSQUIRRELVM vm, const int& value) {
            SQ_PTRS->sq_pushinteger(vm, value);
        }
        ...
        inline void pushValue(HSQUIRRELVM vm, const unsigned int& value) {
            SQ_PTRS->sq_pushinteger(vm, *reinterpret_cast<const int*>(&value));
        }

        template<>
        inline void pushValue(HSQUIRRELVM vm, const unsigned short& value) {
            SQ_PTRS->sq_pushinteger(vm, *reinterpret_cast<const short*>(&value));
        }

        template<>
        inline void pushValue(HSQUIRRELVM vm, const unsigned long& value) {
            SQ_PTRS->sq_pushinteger(vm, *reinterpret_cast<const long*>(&value));
        }

        template<>
        inline void pushValue(HSQUIRRELVM vm, const unsigned char& value) {
            SQ_PTRS->sq_pushinteger(vm, *reinterpret_cast<const char*>(&value));
        }

#ifdef _SQ64
        template<>
        inline void pushValue(HSQUIRRELVM vm, const unsigned long long& value) {
            SQ_PTRS->sq_pushinteger(vm, *reinterpret_cast<const long long*>(&value));
        }
        ...
        inline void pushValue(HSQUIRRELVM vm, const std::wstring& value) {
            SQ_PTRS->sq_pushstring(vm, value.c_str(), value.size());
        }
#else
        template<>
        ...
        inline void pushByPtr(HSQUIRRELVM vm, T* value) {
            ...
            if (value == nullptr) {
                ...
                try {
                    ...
                    SQ_PTRS->sq_createinstance(vm, -1);
                    SQ_PTRS->sq_remove(vm, -2);
                    SQ_PTRS->sq_setinstanceup(vm, -1, (SQUserPointer)(value));
                    SQ_PTRS->sq_settypetag(vm, -1, reinterpret_cast<SQUserPointer>(hashCode));
                }
                catch (std::out_of_range& e) {
                    (void)e;
                    SQ_PTRS->sq_pushuserpointer(vm, (SQUserPointer)(value));
                }
            }
        }
