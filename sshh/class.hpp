    ...
    */
    ...
    protected:
        ...
        void bindVar(const std::string& name, V T::* ptr, HSQOBJECT& table, SQFUNCTION stub, bool isStatic) {
            auto rst = SQ_PTRS->sq_gettop(vm);

            SQ_PTRS->sq_pushobject(vm, table);
            SQ_PTRS->sq_pushstring(vm, name.c_str(), name.size());

            auto vp = SQ_PTRS->sq_newuserdata(vm, sizeof(ptr));
            std::memcpy(vp, &ptr, sizeof(ptr));

            SQ_PTRS->sq_newclosure(vm, stub, 1);
        ...
        static SQInteger varGetStub(HSQUIRRELVM vm) {
            ...
            typedef V T::*M;
            M* memberPtr = nullptr;
            SQ_PTRS->sq_getuserdata(vm, -1, reinterpret_cast<SQUserPointer*>(&memberPtr), nullptr);
            M member = *memberPtr;
        ...
        static SQInteger varSetStub(HSQUIRRELVM vm) {
            T* ptr;
            SQ_PTRS->sq_getinstanceup(vm, 1, reinterpret_cast<SQUserPointer*>(&ptr), nullptr);
    ...
    namespace detail {
        ...
        inline Class popValue(HSQUIRRELVM vm, SQInteger index){
            ...
            Class val(vm);
            if (SQ_FAILED(SQ_PTRS->sq_getstackobj(vm, index, &val.getRaw()))) throw TypeException("Could not get Class from squirrel stack");
